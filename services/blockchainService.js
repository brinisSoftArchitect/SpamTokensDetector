...
class BlockchainService {
  ...
  async parseFromWeb(contractAddress, network) {
    try {
      ...
      // holderConcentrationService is called in tokenAnalyzer.js (STEP 1)
      // blockchainService should NOT call it again to avoid duplicate puppeteer sessions
      // This method only fetches token name/symbol/totalSupply from the main page
      console.log(`${color}📋 [BlockchainService] Fetching token metadata only (holders handled by tokenAnalyzer)${reset}`);
